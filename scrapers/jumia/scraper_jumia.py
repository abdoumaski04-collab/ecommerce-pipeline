...
def scrap_jumia_smartphones(max_pages=5):
    # 🔽 UNIQUEMENT L'URL CHANGE
    base_url = "https://www.jumia.ma/smartphones/?page={}"
    ...
    print(f"📡 Scraping Jumia - Smartphones (max {max_pages} pages)")
    ...
    # Sauvegarde finale
    chemin = "../../data/jumia_smartphones.json"
    with open(chemin, "w", encoding="utf-8") as f:
        json.dump(tous_les_produits, f, indent=2, ensure_ascii=False)
    
    print(f"\n🎉 TOTAL: {len(tous_les_produits)} smartphones sauvegardés dans {chemin}")
    return tous_les_produits

if __name__ == "__main__":
    scrap_jumia_smartphones(max_pages=5)  # Ajustez le nombre de pages